...
- Produces:
  ...
  - `access.build_access(record, external, pi_endpoints: dict[str, list[dict]], vertical_channels: dict[str, str]) -> list[dict]`. Each entry is `{kind, label, url, how}`, and an entry may also carry `datasetId`, `network`, `station`, `channel`, `channelSource`, `instrumentKey`, `endpointId` or `refdes`. Every `pi_portal` route carries `endpointId`, the corpus endpoint's `endpoint_id` (e.g. `"PI-PORTAL-ENDPOINT-d95fe5064a76d7af06"`), because `PIPortalToolkit.browse` needs it for instruments with several endpoints (cross-plan ruling in the final review). Earthscope routes always carry `channel`: the station-metadata vertical channel, else `"HHZ"`. `channelSource` is `"station metadata"` or `"default"` (ruling made during execution).
