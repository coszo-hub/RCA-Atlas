...
def main():
    ...
    for r in records:
        ...
        record_lines=[f'# {r.get("resolved_title") or r["id"]}','','## Citation','',r['citation'],'','## Record','',
            ...
            f'- DOI: {r.get("resolved_doi") or "Not identified"}',f'- Source occurrences: {", ".join(r["source_occurrence_ids"])}']
        if a:
            record_lines += ['', '## Abstract', '', a, '', f'Abstract source: {r["abstract_source_url"]}']
        elif r.get('source_description'):
            record_lines += ['', '## Source description', '', r['source_description'], '',
            ...
            record_lines += ['', '## Abstract availability', '',
                'No formal abstract was retrieved or expected for this resource. See the status and notes below.']
        if r.get('notes'):
            record_lines += ['', '## Notes', '', text(r['notes'])]
        (OUT/'records'/f'{r["id"]}.md').write_text('\n'.join(record_lines)+'\n')
    ...
    lines=['# COSZO literature collection','',f'{len(records)} canonical records from {len(occurrences)} citation occurrences; {len(hashes)} retrieved abstracts.','',
           ...
           '- `records/`: one readable Markdown file for every canonical record, including manuals, books, websites, and unavailable abstracts.',
    ...
    assert {r['id']+'.md' for r in records}=={p.name for p in (OUT/'records').glob('*.md')}
