    ...
    <main className="atlas-main">
      <form className="query-form" onSubmit={ask}>
        <BorderBeam size="md" colorVariant="colorful" strength={0.7} active={!loading}>
          <div className="search-box"><span className="atlas-chip">Atlas</span><textarea value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Research question" rows="2" disabled={loading}/><div className="composer-footer"><span className="composer-chip">Evidence</span><label className="model-select"><span className="visually-hidden">Answer model</span><select value={answerModel} onChange={(event) => setAnswerModel(event.target.value)} disabled={loading}><option value="auto">Auto · free fallback</option><option value="gemini-2.5-flash">Gemini Flash</option><optgroup label="Groq · free"><option value="groq-gpt-oss-120b">GPT-OSS 120B</option><option value="groq-gpt-oss-20b">GPT-OSS 20B</option><option value="groq-qwen3-8-27b">Qwen 3.8 27B</option></optgroup><optgroup label="OpenRouter · free, availability varies"><option value="openrouter-glm-5-2">GLM 5.2</option><option value="openrouter-qwen3-8-27b">Qwen 3.8 27B</option><option value="openrouter-nex-n2-5-pro">Nex N2.5 Pro</option><option value="openrouter-gemma-4-31b">Gemma 4 31B</option></optgroup><option value="gpt-5.4-mini">OpenAI GPT-5.4 Mini</option><optgroup label="OpenAI · API credit required"><option value="gpt-5.6-sol" disabled>GPT-5.6 Sol</option><option value="gpt-5.5" disabled>GPT-5.5</option><option value="gpt-5.5-pro" disabled>GPT-5.5 Pro</option></optgroup></select></label><button type="submit" aria-label="Search Ask Atlas" disabled={loading}>↑</button></div></div>
        </BorderBeam>
      </form>
