    ...
    <main className="atlas-main">
      <form className="query-form" onSubmit={ask}>
        <BorderBeam size="md" colorVariant="colorful" strength={0.7} active={!loading}>
          <div className="search-box"><span className="atlas-chip">Atlas</span><textarea value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Research question" rows="2" disabled={loading}/><div className="composer-footer"><span className="composer-chip">Evidence</span><label className="model-select"><span className="visually-hidden">Answer model</span><select value={answerModel} onChange={(event) => setAnswerModel(event.target.value)} disabled={loading}><option value="auto">Auto</option><option value="gemini-2.5-flash">Gemini Flash</option><option value="groq-gpt-oss-120b">Groq GPT-OSS 120B · free</option><optgroup label="OpenAI · API credit required"><option value="gpt-5.6-sol" disabled>GPT-5.6 Sol</option><option value="gpt-5.5" disabled>GPT-5.5</option><option value="gpt-5.5-pro" disabled>GPT-5.5 Pro</option><option value="gpt-5.4-mini" disabled>GPT-5.4 Mini</option></optgroup></select></label><button type="submit" aria-label="Search Ask Atlas" disabled={loading}>↑</button></div></div>
        </BorderBeam>
      </form>
