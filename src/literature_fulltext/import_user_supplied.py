...
WORK_ID = re.compile(r"^(COSZO-REF-\d{3})(?:[-_.].*)?\.pdf$", re.I)
...
def main() -> int:
    ...
    for path in files:
        ...
        if not match:
            report.append({"timestamp": utc_now(), "file": path.name, "status": "unmapped_filename"})
            continue
        work_id = match.group(1).upper()
        row = toolkit.by_id.get(work_id.casefold())
        if not row:
            report.append({"timestamp": utc_now(), "file": path.name, "work_id": work_id, "status": "unknown_work_id"})
            continue
        cache_root = CACHE / work_id
        if (cache_root / "metadata.json").is_file() and not args.replace:
            report.append({"timestamp": utc_now(), "file": path.name, "work_id": work_id, "status": "already_cached"})
            continue
        try:
            result = toolkit.ingest_user_pdf(row, path)
            metadata = result["cache"]["metadata"]
            report.append({"timestamp": utc_now(), "file": path.name, "work_id": work_id,
