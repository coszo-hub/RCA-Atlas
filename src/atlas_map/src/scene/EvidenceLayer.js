    ...
    for (const set of [...this.sets]) {
      ...
      for (const s of [...set.spikes, ...set.cables]) {
        ...
        if (s.mesh) {
          ...
          u.uColor.value.set((on && s.items.find(x => x.n === hl)?.color) || s.color);   // a merged spike takes the focused item's colour
          u.uAlpha.value = (anyOn && !on ? 0.45 : 1) * (sinking ? sink : Math.min(1, rise * 3));
